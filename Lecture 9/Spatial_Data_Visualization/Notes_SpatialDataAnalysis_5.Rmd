...
- A network connection between countries on a map
...
A new format of geo-referenced data.
...
```{r}
d = read_csv("Lecture 9/data/ucdp-prio-acd-221.csv")
```

## Extract Entities of Interest

Extract country names and find the ISO3c codes using the package `countrycode`. 

A "lazy" approach. The best approach will be a locally customized dataset of country names.
...
To draw the network, we need points representing countries' locations. 

Using the map we already have: 

We find the geographic "centroids" for each country using functions from the `sf` package.

```{r}
# Calculate the countries' "centroids"
# Use: st_centroid to calculate the centroid of each geographical unit
# Each country will get a point
...
To check whether the geogrphic centroids are correctly specified:

We plot a world map along with the points of centroids. 

Check if the maps make sense and what is interesting/ unintuitive.

```{r}
# Visualize the centroids of each country 
world_t |>
  ggplot() +
  geom_sf() +
...
Now we have the locations of each country (using their geographic centroids). 

Merge them with the conflict dataset

Basically, connect countries with conflicts. 

```{r}
# Get a dataset with only countries' identifiers and locations of their centroids
# Further subset the dataset to get the longtitude and latitude of the centroid
...
# A new dataset that merges the conflict incidents with the centroids of the two countries involved
# Use left joins
d_t_s_2 = d_t_s |>
...
# Inital figure that links the centroids
# 1. Plot the world map
# 2. Stack ther points on the map
# geom_cuve to curve the linkage line
ggplot() +
  geom_sf(data = world_t) +
...
ggplot() +
  ...
  geom_curve(data = d_t_s_4, aes(x = centroid_xcoord_a, # start of the lines
                                 y = centroid_ycoord_a, 
                                 xend = centroid_xcoord_b, # end of the lines
                                 yend = centroid_ycoord_b, 
                                 linewidth = n), # represents the no. of conflicts
             curvature = 0.2, color = "red", alpha = 0.5)

# Want: Only show top 10 countries with conflict
d_top10_n_conflict <- d_t_s_4 |>
  slice_max(n, n = 6) # we get 10 countries when n=6
