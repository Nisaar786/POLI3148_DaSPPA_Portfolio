...
- More advanced spatial data analysis
...
#Libraries containing a lot of map data 
...
```{r}
# A new type of world map dataset
...
```{r}
# use geom_sf since its is a sf type dataset
...
```{r}
ggplot() + 
  geom_sf(data = world) + # up til here is plotting the map as a background
  geom_point(data = d_event_2021, # plot the conflict data
...
```{r}
# 1. Filter Africa map from the world map 
# 2. Filter Africa's conflict data from the d_event_2021
ggplot() + 
...
world_m_africa = world_m |>
  ...
  mutate(geometry = st_transform(geometry, 3857)) # Specify the projection of the map. Essential

# Fill in NA n_conflict
...
# The text location of the countries is still according to the map 
...
In cross-country analysis, the most trouble is the matching of country identifiers. 

One thing that goes unnoticed in the above steps is that some contries "fell out" of the study siliently.

We have a handy tool to help match countries from different datasets correctly.
