...
-   Systematically interpret large amounts of text

-   Tedious, BUT ESSENTIAL part of political social data analytics!
...
*Ultimately, the instructions should make sense to both humans and AIs.*

## A look at the Example Instructions
...
-   Improve Instructions using ChatGPT itself!


## Finalize the Instructions

The finalized instructions is saved in `data/instructions_improved_2.txt`.
...
instructions <- readLines("data/input/instructions_improved_2.txt") |>
  str_c(collapse = "\n")
cat(instructions) # Print the improved instruction
```

## How Else Can ChatGPT Help with Instructions?

-   Clarify concepts (Specify concretely)
...
Start with half-cooked instructions and ask ChatGPT to help you build it up. 

You will be pleasantly surprised how well ChatGPT does.
...
-   Conflicts between two coders? It happens a lot! (Subjective on a person)
...
Make a smaller "training" set for demostration

```{r}
# Create a training dataset with 40 tweets
set.seed(12)
d_train <- d |> 
...
How to use ChatGPT to annotate one document.
...
# cat is simply print function but print nicely 
cat(to_annotate_id, "\n\n", to_annotate_text, "\n\n", to_annotate_raLabel)
```

## Annotate one Doc: Communicate with the ChatGPT API

-   We are going to ask ChatGPT to label a tweet
...
library(jsonlite) # Package to parse results sent by ChatGPT, allow to read Json format
...
response <- POST(
  ...
  body = list(
    temperature = 0, 
    # temperature:0 to 1. How "creative" you want ChatGPT to be. Smaller = less creative.
    messages = list(
      list(role = "system", content = instructions), # The content 
...
# Save the tweet according to the unique ID
...
# This is where ChatGPT's stores the answer to your request
content(response)$choices[[1]]$message$content # only gets you only the content
...
-   Run chatgpt for 40 tweets

-   Check out what kind of output chatgpt will give 
...
*How good do you think this result is? 

It agrees with the human annotator in 3/4 of the tasks.*
